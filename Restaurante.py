...
class AplicacionConPestanas(ctk.CTk):
    ...
    def actualizar_treeview(self):
        ...
        for ingrediente in self.stock.lista_ingredientes:
            # Mostrar la cantidad con un decimal en la interfaz
            try:
                cantidad_mostrar = f"{float(ingrediente.cantidad):.1f}"
            except (ValueError, TypeError):
                cantidad_mostrar = str(ingrediente.cantidad)

            self.tree.insert("", "end", values=(ingrediente.nombre, ingrediente.unidad, cantidad_mostrar))    
    ...
    def tarjeta_click(self, event, menu):
        ...
        if suficiente_stock:
            for ingrediente_necesario in menu.ingredientes:
                for ingrediente_stock in self.stock.lista_ingredientes:
                    if ingrediente_necesario.nombre == ingrediente_stock.nombre:
                        # Restar con floats y mantener decimales
                        try:
                            nueva_cant = float(ingrediente_stock.cantidad) - float(ingrediente_necesario.cantidad)
                        except (ValueError, TypeError):
                            nueva_cant = 0.0
                        # Evitar negativos y redondear a 1 decimal para consistencia visual
                        if nueva_cant < 0:
                            nueva_cant = 0.0
                        ingrediente_stock.cantidad = round(nueva_cant, 3)
    ...
    def validar_cantidad(self, cantidad):
        # Aceptar enteros y decimales positivos
        try:
            val = float(cantidad)
            if val >= 0:
                return True
            ...
            pass
        CTkMessagebox(title="Error de Validación", message="La cantidad debe ser un número positivo (ej. 1 o 0.3).", icon="warning")
        return False
    ...
    def ingresar_ingrediente(self):
        ...
        # Normalizar antes de crear el objeto: Stock.agregar_ingrediente también normaliza,
        # pero normalizar tempranamente ayuda a mantener consistencia en la UI.
        try:
